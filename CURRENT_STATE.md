...
- The owner requested v9 runtime enablement on 2026-08-26 after the review boards were available. The first release-closure commit keeps the extension fail-closed as `pending_windows_renderer_ci`; the runtime loader verifies byte counts and SHA-256 values and exposes `road-gaze/left` and `road-gaze/right` only after a separate post-CI promotion records `runtime_validation=passed_windows_renderer_qa`, `runtime_approved=true`, and `runtime_use=true`.
