...
- Added `WK-INTERACTION-CAR-RIDE-ROAD-GAZE-CANDIDATE-v9`: two independently produced left/right road-gaze sequences, 18 frames per direction, aligned to the approved v8 six-frame wheel phase. The approved v8 car and wheel pixels are unchanged below the edited head/neck region.
- The v9 extension is deliberately fail-closed. The runtime loader verifies byte counts and SHA-256 values and will expose `road-gaze/left` and `road-gaze/right` only after the extension itself records `runtime_validation=passed_windows_renderer_qa`, `runtime_approved=true`, and `runtime_use=true`. Until owner visual review, the existing 222-frame v8 runtime map is unchanged.
- Added `WK-AUTONOMOUS-SIDE-PRONE-FRONT-CANDIDATE-v1`: a 12-frame calm loop that preserves the approved V3R1 side-prone body while using the approved V4 forward-facing head identity. It remains `runtime_approved=false`, `runtime_use=false`, and `autonomous_binding_enabled=false`; the rear-look-to-front and front-to-rear bridge sequences still have to be produced and approved before any autonomous binding.
...
- A repository-wide, fail-closed mirror audit covers 25 action packages. None currently declares an explicit safe mirror contract, so no full-frame mirror derivative is generated or runtime-integrated. Native opposite-direction car/broom art remains authoritative; tail direction, paw handedness, prop orientation, text/effects, and approved pixel identity are never inferred to be mirror-safe.
- Local Python asset/provenance regression passes 53 tests. This Linux checkout has no .NET SDK or Windows renderer, so the C# build, Desktop test suite, transparent WPF playback, and owner visual QA remain pending on Windows. No Windows approval is inferred from asset tests.
