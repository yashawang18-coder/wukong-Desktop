...
- One mirror flag is captured by `PetMotionRequest` and remains fixed through every phase and command terminal hold. A native left/right patrol updates the facing inherited by the next eligible action, avoiding per-frame or mid-action reversal.
- Magic, petrified coins, broom effects, car ride/native road-gaze directions, the already paired patrol directions, deprecated assets, and actions with multiple native direction groups remain excluded.
- The control-panel frame viewer exposes `镜像查看` only for eligible actions. Existing visual/runtime approval fields are unchanged; Windows owner visual review of the orientation variants remains pending.
- Desktop build and the mirror regression tests pass locally. This work remains uncommitted on `agent/coin-assets-v19-runtime-fix` together with the already present coin/chat candidate changes.

## Petrified coin v19 local runtime candidate - 2026-09-07

- Imported the eight owner-supplied masters from `dog_coin_assets_8pack_final_v19` into the versioned `petrificus_coin/v19/` path: front and back faces for vivid, flat, faded, and exhausted states.
- All masters are 1024 x 1024 RGBA, share the exact alpha mask and `(62,70)-(962,952)` visible bounds, and preserve the bottom-center `(512,952)` anchor. Visible RGBA and alpha values are unchanged; RGB was cleared only under fully transparent pixels to avoid WPF resampling fringe.
- Four nine-frame front-to-back flip sequences were derived deterministically with premultiplied-alpha horizontal compression using widths `900, 845, 672, 415, 112, 415, 672, 845, 900`. Each sequence starts and ends with an exact pixel match to its v19 front/back face; back-to-front playback continues to use the existing reverse strategy.
- `coin-manifest.json`, the Petrificus initial coin frame, and `coin-checksums.sha256` now point only to the 44 v19 runtime-candidate PNGs. The prior 44 coin PNGs remain immutable historical evidence and are not referenced by the active coin manifest.
- Static validation passed for decode, dimensions, alpha, shared geometry, transparent RGB, source provenance, frame counts, checksums, and flip endpoints. Windows transparent WPF visual QA is still pending.
- Candidate state remains `visual_approved=false`, `runtime_validation=pending_windows_renderer_qa`, `runtime_approved=false`, `runtime_use=false`, `production_asset=false`, and `prototype_use=true`. Existing owner-only `PrototypePreview` sources and the two-thirds coin presentation scale are unchanged.
- Repository stage is local uncommitted candidate on `agent/coin-assets-v19-runtime-fix`; no production registry or normal/autonomous/model route is enabled.
...
- A repository-wide, fail-closed pixel-derivative audit covers the historical action packages. None declares an explicit safe contract for generating new mirrored PNG bytes, so no full-frame mirror file derivative is generated or approved. This remains distinct from the 2026-09-09 allowlisted WPF whole-frame orientation transform; native opposite-direction car/broom/patrol art remains authoritative.
...
- Replaced unconditional periodic speech with a state-driven initiative decision. Hunger, thirst, social need, boredom/play, curiosity, energy/rest, the current Episode, temperament, relationship initiative acceptance, cooldown, quiet hours, stress, current behavior, petrification, and expanded chat all participate. State urgency selects a 1-5 minute evaluation interval, while a successful utterance starts a 7-20 minute state-sensitive cooldown. Initiative text is a constrained local rule sentence and appears only in the transient speech bubble; it does not enter conversation history and does not call a model.
